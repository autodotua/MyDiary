...
namespace MyDiary.Models.Converters
{
    public class Json2DArrayConverter : JsonConverterFactory
    {
        ...
        class Array2DConverterInner<T> : JsonConverter<T[,]>
        {
            ...
            public override void Write(Utf8JsonWriter writer, T[,] array, JsonSerializerOptions options)
            {
                ...
                for (var i = rowsFirstIndex; i <= rowsLastIndex; i++)
                {
                    ...
                    for (var j = columnsFirstIndex; j <= columnsLastIndex; j++)
                        WriteOrSerialize(_valueConverter, writer, array[i, j], options);
            ...
            {
                var list = JsonSerializer.Deserialize<List<List<T>>>(ref reader, options);
                return list == null ? null : To2D(list);
            }
        }

        public static T[,] To2D<T>(List<List<T>> source)
        {
